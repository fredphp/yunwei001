...
$cfgarr = array(
	'cqssc' => array(
		...
		'url' => 'http://api.api68.com/CQShiCai/getBaseCQShiCai.do?lotCode=10060',//采集URL
	),
	'gdkl' => array(
		'name' => '广东快乐10分',
		'starttime' => '',//开奖时间 提前20分
	...
	'jnd28' => array(//加拿大28，每三分半钟开一期，每天维护时间为: 晚上20:00点到21:30点，周一可能会有延迟
		...
		'template' => array('fantan', 'pc28'),//操作模板
		'url' => 'https://leyinga.com/?m=api&c=trend&a=getMoreLottery&lottery_type=3',//采集URL
	),
	'pk10' => array(
		'name' => '北京赛车',
		'starttime' => '',//开奖时间
		...
		'url' => 'https://leyinga.com/?m=api&c=trend&a=getMoreLottery&lottery_type=2',//采集URL
	),
	'teqdd' => array(//土耳其蛋蛋 自开奖 每天从早上00：00至23:59：59每2分钟 30秒一期
		'name' => '土耳其蛋蛋',
	...
	'jsssc' => array(//土耳其蛋蛋 自开奖 每天从早上00：00至23:59：59每1分钟 30秒一期
		'name' => '极速时时彩（番）',
		'starttime' => '00:0:03',//开奖时间 提前20分
		'endtime' => '23:59:59',//关奖时间 推迟20分
		'time' => 35,//结算刷新时间
...
function gohaoma_url($gameid, $game) {
	...
	$result = fileget_content($url, '', '', $url, $header, 10);
	...
	if(strpos($url,'api68.com') !== false){
		...
		switch ($game){
			...
			case 'jnd28':
				...
				$content['current']['haoma'] = intval($haomaarr[0]).','.intval($haomaarr[1]).','.intval($haomaarr[2]);//开奖号码
				$next_qishu = $content['current']['qishu'] + 1;//下期期数
				$next_sendtime = strtotime($open_time) + 210;//3分半一期
				break;
			case 'cqssc':
				//重庆时时彩
				$content['current']['haoma'] = $haoma;//开奖号码
		...
		switch ($game){
			...
			case 'pk10':
				...
				if ($timearr[1] == '23:57:00') {//返回的是最后一期
					...
					$next_sendtime = strtotime($open_time) + 1200;//当前返回时间加5分钟
				}
				break;
			case 'jnd28':
				//加拿大28
			...
			case 'cqssc':
				...
				if ($h == 2) {//返回的是最后一期
					...
					$next_sendtime = strtotime($open_time) + 600;//当前返回时间加5分钟
				} else {
					$next_sendtime = strtotime($open_time) + 1200;//当前返回时间加10分钟
				}
				break;
		}
		$content['next']['qishu'] = htmlspecialchars(addslashes($next_qishu));//下期期数
