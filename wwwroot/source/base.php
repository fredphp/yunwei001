<?php
// ★ 立即开启输出缓冲 — 防止任何PHP警告/空格在header()之前输出导致乱码
ob_start();
...
if (!is_dir(CACHE_PATH . 'caches_template')) {
        @mkdir(CACHE_PATH . 'caches_template', 0777, true);
}
if (!is_dir(CACHE_PATH . 'caches_template/default')) {
        @mkdir(CACHE_PATH . 'caches_template/default', 0777, true);
}
...
// ★ 关闭错误显示，所有错误写日志，防止警告输出到页面导致乱码
error_reporting(E_ALL & ~E_DEPRECATED & ~E_NOTICE);
ini_set('display_errors', '0');
ini_set('display_startup_errors', '0');
ini_set('log_errors', '1');
// ★ 始终注册错误处理器，确保所有错误都不输出到页面（无论 errorlog 配置如何）
set_error_handler('my_error_handler_silent');
// 设置本地时差
