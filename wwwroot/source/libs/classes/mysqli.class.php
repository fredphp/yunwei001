...
final class db_mysqli {
        ...
        public function connect() {
                ...
                // 设置字符集（MySQL 8.0 兼容，支持 utf8mb4）
                $charset = isset($this -> config['charset']) ? $this -> config['charset'] : 'utf8mb4';
                if (function_exists('mysqli_set_charset') && $charset) {
                        $this -> link -> set_charset($charset);
                }
                if ($this -> version() > '4.1') {
                        // connection/results 用 utf8mb4 保证数据正确显示，client 保持 binary 兼容 addslashes 转义机制
        ...
        public function fetch_next($type = MYSQLI_ASSOC) {
                if (!is_object($this -> lastqueryid)) { $this -> free_result(); return false; }
        ...
        public function free_result() {
                if ($this -> lastqueryid instanceof mysqli_result) {
